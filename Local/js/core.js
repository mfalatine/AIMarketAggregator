import { CLI_MODELS } from './cli-models.js';
...
export const MODELS = [...CLI_MODELS];
...
export const DEFAULT_PROFILES = [
  {
    ...
    depth: 'standard', transport: 'cli', cliModelId: 'cli-codex', modelId: 'cli-codex', instructions: 'Prioritize actionable catalysts and clearly separate confirmed facts, forecasts, market-implied values, and interpretation.'
  },
  {
    ...
    depth: 'quick', transport: 'cli', cliModelId: 'cli-codex-spark', modelId: 'cli-codex-spark', instructions: 'Focus on developments since the prior close and events scheduled before the next close.'
  },
  {
    ...
    depth: 'deep', transport: 'cli', cliModelId: 'cli-claude-opus-4-8', modelId: 'cli-claude-opus-4-8', instructions: 'Explain what changed during the period, what the market is pricing now, and the strongest bull and bear cases for next week.'
  },
  {
    ...
    depth: 'standard', transport: 'cli', cliModelId: 'cli-claude-sonnet-4-6', modelId: 'cli-claude-sonnet-4-6', instructions: 'Emphasize expectation gaps, guidance, valuation implications, and read-throughs to peers.'
  },
  {
    id: 'earnings-futures', name: 'Mega-Cap Earnings Futures', description: 'Pre-earnings ES/NQ setup: direction, gap risk, severity score, key levels',
    topicIds: ['geopolitical', 'earnings_notable', 'vix_sentiment', 'sp500_technicals'],
    watchlist: ['AAPL', 'MSFT', 'NVDA', 'GOOGL', 'AMZN', 'META'], coverageIds: ['price_moving_news', 'upcoming_earnings', 'options_unusual'],
    depth: 'standard', transport: 'cli', cliModelId: 'cli-codex', modelId: 'cli-codex', instructions: 'Pre-earnings futures prep for the mega caps on the watchlist that report next. Answer in this order: (1) macro and geopolitical headwinds or tailwinds since their last reports, including strategic moves by global competitors such as rival earnings, IPOs, or major announcements; (2) expected net impact on S&P 500 (ES) and Nasdaq 100 (NQ) futures given index weights; (3) directional bias, bullish or bearish, and whether major price expansion up or down is expected; (4) whether the reaction is likely to create a price gap at the open or session restart; (5) a move severity score from 0 (negligible, sideways) to 5 (massive volatility, extreme range expansion) with a one-line rationale; (6) specific ES and NQ support and resistance levels to watch. Cite the options-market implied move where available. Be direct and focused on trading implications.'
  }
];
...
export function modelsForTransport(transport = 'cli') { return MODELS.filter(model => model.transport === transport); }
